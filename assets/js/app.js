...
// queryURLBase is the start of our API endpoint. The searchTerm will be appended to this when the user hits the search button
...
=======
	...
	$.ajax({
		...
		for (var i = 0; i < 10; i++) {
			...
			if (NYTData.response.docs[i].headline !== "null") {
				$("#article-well-" + articleCounter)
					.append("<h4 class='articleHeadline'> <span class='label label-primary'>" +
						articleCounter + "</span>.  " + NYTData.response.docs[i].headline.main + "</h4>"
					);
			...
			$("#articleWell-" + articleCounter )
				.append(
					"<a href='" + NYTData.response.docs [i].web_url + "'>" + NYTData.response.docs [i].web_url + "</a>"
				);
...
>>>>>>> f71064f059ac6fcbe78fcb86b437f6fe19588d9c
})